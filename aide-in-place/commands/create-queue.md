---
description: "Generate a prioritized queue of the next batch of migration work items."
---

# Create Queue — In-Place Migration

Generate the next batch of prioritized migration work items.
...
6. **Migration awareness** — for migration items, include the `[MIGRATION X→Y]` prefix and reference the specific migration stage from the roadmap
7. **Consistent format** — each item must follow this format so other commands can parse it:
   ```
...
Select an item from the queue and start a **new chat session**. Run `/speckit.aide.create-item` with the item description to create a detailed migration work item specification.
